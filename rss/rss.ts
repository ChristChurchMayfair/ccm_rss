...
export function writeEpisodesToFile(
    config: RssChannelConfig,
    episodes: Episode[],
...
export const createXml = (
    ...
    const category = channel.ele("itunes:category", {
        text: config.category.major,
    });
    ...
    episodes
        .filter((episode) => (episode.embargoed ?? false) !== true)
        .forEach((episode) => {
...
const createItem = (
    ...
    item.ele(
        "pubDate",
        formatDate(parse(episode.releaseDate, inputDateFormat, new Date()))
    );
    item.ele("itunes:duration", formatDuration(episode.durationInSeconds));
    if (episode.season != null) {
        item.ele("itunes:season", episode.season.toString());
    }
    item.ele("enclosure", {
